...
import {
  ...
  normalizeCurrentUrlForLinkClassification,
  normalizeRouaultUrl,
  stripBasePathFromPathname,
  type CurrentDocumentClassificationContext,
...
export const assertRouteClassificationModeAllowed = (options: {
  ...
  if (options.runtimeEnvironment === 'production' && options.mode.kind !== 'manifest-loaded') {
    throw new LinkClassificationContractError(
      ...
      'production runtime cannot use dev/test route classification fallback.',
    );
  }
};
...
const requiresCurrentUrlForClassification = (href: string): boolean =>
  !href.startsWith('/') &&
  !href.startsWith('//') &&
  !HAS_EXPLICIT_SCHEME_RE.test(href) &&
  RELATIVE_URL_REQUIRES_CURRENT_URL_RE.test(href);
...
const annotation = (options: {
  ...
  return {
    ...
    kind: options.kind,
    surface: options.surface,
    routerInterceptionPolicy: resolveRouterInterceptionPolicy(
      options.kind,
...
export const classifyLinkHref = (
  options: ClassifyLinkOptions & { readonly href: string },
): ResolvedLinkAnnotation => {
  ...
  if (!unsafe.ok)
    return annotation({
      ...
      sanitizedHref: options.href.trim(),
      kind: 'unsafe',
      surface,
  ...
  try {
    base =
      options.currentUrl && needsCurrentUrl
        ? normalizeCurrentUrlForLinkClassification({
            siteUrlContext: options.siteUrlContext,
            currentUrl: options.currentUrl,
            context: toCurrentDocumentClassificationContext(options.routeClassificationMode),
          })
  ...
  if (!isPathnameInsideBasePath(resolved.pathname, options.siteUrlContext.basePath))
    return annotation({
      rawHref: options.href,
      sanitizedHref,
      renderHref: `${resolved.pathname}${resolved.search}${resolved.hash}`,
  ...
  if (
    ...
    const currentPathname = stripBasePathFromPathname(
      current.pathname,
      options.siteUrlContext.basePath,
    );
    const currentDocumentIsInternal =
      options.routeClassificationMode.kind === 'manifest-loaded'
    ...
    if (currentDocumentIsInternal) {
      return annotation({
        rawHref: options.href,
        sanitizedHref,
        renderHref: resolved.hash,
        kind: 'internal-fragment',
        surface,
        classificationOptions: options,
      });
    }
  ...
  if (options.routeClassificationMode.kind === 'manifest-loaded') {
    if (options.routeClassificationMode.isInternalDocumentPathname(pathname)) {
      return annotation({
        rawHref: options.href,
        sanitizedHref,
        renderHref: applyBasePathToRenderHref({
          pathname,
    ...
    options.routeClassificationMode.recordDiagnostic({
      reason: 'dev-test-missing-manifest-fallback',
      pathname,
    });
  }
  const resource =
    isDefaultInternalResourcePathname(pathname) ||
    options.isInternalResourcePathname?.(pathname) === true;
  return annotation({
    rawHref: options.href,
    sanitizedHref,
    renderHref: applyBasePathToRenderHref({
    ...
    kind: resource ? 'internal-resource' : 'internal-resource',
    surface,
    classificationOptions: options,
  });
};
