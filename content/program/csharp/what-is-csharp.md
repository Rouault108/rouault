...
この関係を理解するには、CLI（Common Language Infrastructure）、CTS（Common Type System）、CLS（Common Language Specification）、ランタイム実装、ライブラリ群を区別する必要がある。CLIはECMA-335が定義する共通言語基盤であり、CLIを対象とするC#コンパイラでは出力する共通中間言語（Common Intermediate Language、以下IL）、メタデータとメタデータが従う共通の形式を与える。CTSは、その基盤上で型をどのように表現し、相互に関係付けるかという共通規則を定める。CLSは、そのうち言語間で無理なく共有できる機能の範囲を定める。たとえば、あるC#の型が他の.NET言語から利用しやすいかという問題は、CLSとの関係で捉えられる。.NETにおけるランタイムは、この共通基盤を具体的な実行系として実装し、アセンブリの読み込み、検証、JITコンパイル、ガベージコレクションなどを担う。これに対して.NETのライブラリ群は、`object`、`string`、コレクション、例外、I/O、非同期処理などのAPIを提供する。したがって、C#のソースコードは、コンパイラによってCLI上の形式へ変換され、ランタイムによって実行され、ライブラリによって具体的な機能を得るという三層の関係で理解できる。[^2][^5]
