...
C#のソースコードは、コンパイラによって字句列、構文木、意味情報として処理され、アセンブリ、CIL、メタデータ、シンボル情報へ変換される。生成されたアセンブリは、共通言語ランタイム（CLR）によるロード、検証、JITコンパイル、実行の対象となる。C#プログラムの記述、変換、実行は、ソースコード、コンパイル単位、アセンブリ、実行基盤への受け渡しから成る連続した過程として位置付けられる。[^1][^2][^3]
