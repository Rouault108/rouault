...
export class Slider extends LitElement {
  ...
  private _normalize(): void {
    ...
    const normalizedStep =
      typeof this.step === 'number' && isFinite(this.step) && this.step > 0 ? this.step : 1;
  ...
  private _getAriaLabel(): string {
    ...
    if (!this._hasWarnedMissingLabel) {
      ...
      console.warn(
        '[ui-slider] `label` は必須です。空の場合は "Slider" をフォールバック使用します。',
      );
    }
    return 'Slider';
  }
    ...
    return html`
      ...
      <div class="slider-container">
        ...
        <div class="track" part="track">
          <!-- フィル（アクティブ部分） -->
          <div class="fill" part="fill" style="width: ${fillWidth}"></div>

          <!-- Thumb（つまみ） -->
          <div class="thumb" part="thumb" style="left: ${thumbLeft}"></div>
        </div>
      </div>
