...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (links?.length !== 5) {
      throw new Error(
        `5つのリンクを期待していましたが、実際には ${String(links?.length ?? 0)}個でした`,
      );
    }
    ...
    if (activeLinks && activeLinks.length > 0) {
      throw new Error(
        `アクティブリンクが0件であることを期待していましたが、実際には ${String(activeLinks.length)}件でした`,
      );
    }
    ...
    if (firstLink.getAttribute('href') !== '#intro') {
      throw new Error(
        `href="#intro" を期待していましたが、実際には "${firstLink.getAttribute('href') ?? 'null'}" でした`,
      );
    }
...
export const WithActiveItem: Story = {
  render: () => html`
    <div style="width: 200px;">
      <ui-toc id="active-toc" .headers="${flatH2Headers}" active-id="implementation"></ui-toc>
    </div>
  `,
  play: async ({ canvasElement }) => {
    ...
    if (allActiveLinks?.length !== 1) {
      throw new Error(
        `1つのアクティブリンクを期待していましたが、実際には ${String(allActiveLinks?.length ?? 0)}個でした`,
      );
    }
...
export const Nested: Story = {
  render: () => html`
    <div style="width: 220px;">
      <ui-toc id="nested-toc" .headers="${nestedHeaders}" active-id="setup"></ui-toc>
    </div>
  `,
  play: async ({ canvasElement }) => {
    ...
    for (const idx of h2Indices) {
      ...
      if (level !== '0') {
        throw new Error(
          `インデックス ${String(idx)} の H2 アイテムは --level: 0 を持つべきですが、実際には "${level ?? 'undefined'}" でした`,
        );
      }
    }
    ...
    for (const idx of h3Indices) {
      ...
      if (level !== '1') {
        throw new Error(
          `インデックス ${String(idx)} の H3 アイテムは --level: 1 を持つべきですが、実際には "${level ?? 'undefined'}" でした`,
        );
      }
    }
...
export const DeepNesting: Story = {
  render: () => html`
    <div style="width: 240px;">
      <ui-toc id="deep-toc" .headers="${deepNestedHeaders}" active-id="frontend"></ui-toc>
    </div>
  `,
  play: async ({ canvasElement }) => {
    ...
    for (const idx of h4Indices) {
      ...
      if (level !== '2') {
        throw new Error(
          `インデックス ${String(idx)} の H4 アイテムは --level: 2 を持つべきですが、実際には "${level ?? 'undefined'}" でした`,
        );
      }
    }
  ...
  play: async ({ canvasElement }) => {
    ...
    for (let i = 1; i < links.length; i++) {
      if (links[i]?.hasAttribute('aria-current')) {
        throw new Error(
          `インデックス ${String(i)} のリンクは aria-current を持つべきではありません`,
        );
      }
    }
  },
...
export const ActiveLast: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (lastLink.getAttribute('href') !== '#conclusion') {
      throw new Error(
        `最後のリンクの href が "#conclusion" であることを期待していましたが、実際には "${lastLink.getAttribute('href') ?? 'null'}" でした`,
      );
    }
  },
};
...
export const SingleItem: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (links?.length !== 1) {
      throw new Error(
        `1つのリンクを期待していましたが、実際には ${String(links?.length ?? 0)}個でした`,
      );
    }
    ...
    if (level !== '0') {
      throw new Error(
        `アイテムの --level が 0 であることを期待していましたが、実際には "${level ?? 'undefined'}" でした`,
      );
    }
...
export const EmptyHeaders: Story = {
  parameters: {
    docs: {
      description: {
        story:
          '⚠️ **境界条件**: `headers` が空の配列の場合、何も表示されません（`nav` 要素も生成されません）。',
      },
    },
  },
  render: () => html`
  ...
  play: async ({ canvasElement }) => {
    ...
    if (links && links.length > 0) {
      throw new Error(
        `リンクが0件であることを期待していましたが、実際には ${String(links.length)}個でした`,
      );
    }
  },
};
...
export const OnlySubheadings: Story = {
  parameters: {
    docs: {
      description: {
        story:
          '⚠️ **境界条件**: H3 のみの場合、最小レベル(H3=3)を基準に全アイテムが `--level: 0` に正規化されます。',
      },
    },
  },
  render: () => html`
  ...
  play: async ({ canvasElement }) => {
    ...
    if (activeLink.getAttribute('href') !== '#config') {
      throw new Error(
        `アクティブリンクの href が "#config" であることを期待していましたが、実際には "${activeLink.getAttribute('href') ?? 'null'}" でした`,
      );
    }
  },
};
...
export const SparseLevels: Story = {
  parameters: {
    docs: {
      description: {
        story:
          '⚠️ **境界条件**: H1, H3, H5 のような歯抜け構造。正規化は `level - minLevel` で算出するため、インデントが飛び飛びになります。',
      },
    },
  },
  render: () => html`
  ...
  play: async ({ canvasElement }) => {
    ...
    if (listItems?.length !== 3) {
      throw new Error(
        `3つのアイテムを期待していましたが、実際には ${String(listItems?.length ?? 0)}個でした`,
      );
    }
    ...
    if (h1Level !== '0') {
      throw new Error(
        `H1の --level が 0 であることを期待していましたが、実際には "${h1Level ?? 'undefined'}" でした`,
      );
    }
    ...
    if (h3Level !== '2') {
      throw new Error(
        `H3の --level が 2 であることを期待していましたが、実際には "${h3Level ?? 'undefined'}" でした`,
      );
    }
    ...
    if (h5Level !== '4') {
      throw new Error(
        `H5の --level が 4 であることを期待していましたが、実際には "${h5Level ?? 'undefined'}" でした`,
      );
    }
  },
};
...
export const LongText: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (hostRect.width > 220) {
      // 200px コンテナ + 余裕
      throw new Error(
        `TOC の幅 (${String(Math.round(hostRect.width))}px) がコンテナを超えています`,
      );
    }
    ...
    const h3Label = toc.shadowRoot?.querySelector<HTMLElement>(
      '.toc-link-label[data-heading-id="long-h3"]',
    );
    ...
    const h4Label = toc.shadowRoot?.querySelector<HTMLElement>(
      '.toc-link-label[data-heading-id="long-h4"]',
    );
    ...
    if (h4Style.textOverflow !== 'ellipsis') {
      throw new Error(
        `H4 の text-overflow=ellipsis を期待していましたが、実際には "${h4Style.textOverflow}" でした`,
      );
    }
...
export const ClickToActivate: Story = {
  render: () => html`
    <div style="display: grid; grid-template-columns: 220px minmax(280px, 1fr); gap: 1.5rem;">
      <div style="width: 200px;">
        <ui-toc id="click-toc" .headers="${flatH2Headers}" active-id="intro"></ui-toc>
      </div>
  ...
  play: async ({ canvasElement }) => {
    ...
    if (!targetHeading)
      throw new Error('ドキュメント内に #implementation 見出しが見つかりませんでした');
    ...
    const conclusionLink =
      toc.shadowRoot?.querySelector<HTMLAnchorElement>('a[href="#conclusion"]');
...
export const ActiveIdNotFound: Story = {
  parameters: {
    docs: {
      description: {
        story:
          '⚠️ **境界条件**: `activeId` がどの見出し ID にも一致しない。どのリンクもアクティブにならず、正常にレンダリングされます。',
      },
    },
  },
  render: () => html`
    <div style="width: 200px;">
      <ui-toc id="notfound-toc" .headers="${flatH2Headers}" active-id="nonexistent-id"></ui-toc>
    </div>
  `,
  play: async ({ canvasElement }) => {
    ...
    if (activeLinks && activeLinks.length > 0) {
      throw new Error(
        `ID が見つからない時はアクティブリンクが 0 件であることを期待していましたが、実際には ${String(activeLinks.length)}件でした`,
      );
    }
    ...
    if (activeClassLinks && activeClassLinks.length > 0) {
      throw new Error(
        `is-active クラスを持つリンクが 0 件であることを期待していましたが、実際には ${String(activeClassLinks.length)}件でした`,
      );
    }
  },
};
...
export const AccessibilityStructure: Story = {
  render: () => html`
    <div style="width: 200px;">
      <ui-toc id="a11y-toc" .headers="${nestedHeaders}" active-id="implementation"></ui-toc>
    </div>
  `,
  play: async ({ canvasElement }) => {
    ...
    allLinks.forEach((link, idx) => {
      ...
      if (!href?.startsWith('#')) {
        throw new Error(
          `${String(idx)}番目のリンクの href は "#" で始まるべきですが、実際には "${href ?? 'null'}" でした`,
        );
      }
    });
    ...
    if (firstLinkStyle.display !== 'flex') {
      throw new Error(
        `toc-link は flex レイアウトである必要がありますが、実際には "${firstLinkStyle.display}" でした`,
      );
    }
...
export const KeyboardAndTouchTarget: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (!Number.isFinite(minHeight) || minHeight < 24) {
      throw new Error(
        `min-height >= 24px を期待していましたが、実際には ${String(minHeight)}px でした`,
      );
    }
...
export const AllStates: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (allTocs.length !== 8) {
      throw new Error(
        `8つの目次コンポーネントを期待していましたが、実際には ${String(allTocs.length)}個でした`,
      );
    }
    ...
    const secondActiveLinks = secondToc.shadowRoot?.querySelectorAll('[aria-current="location"]');
...
- アクティブインジケーターが \`background-color\` ではなく \`border: Highlight\` で表示されることを確認
  ...
  play: async ({ canvasElement }) => {
    ...
    if (
      ...
      throw new Error(
        `アニメーション軽減モードでは duration 0.01ms を期待していましたが、実際には "${animationDuration}" でした`,
      );
    }
    ...
    if (window.matchMedia('(forced-colors: active)').matches) {
      if (beforeStyle.borderStyle === 'none') {
        throw new Error(
          'ハイコントラストモードではアクティブインジケーターに枠線（border）が表示されるべきです',
        );
      }
    }
  },
...
export const AutoScrollOnlyWhenOutOfView: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const targetLink = toc.shadowRoot?.querySelector<HTMLAnchorElement>(
      'a[href="#implementation"]',
    );
    ...
    if (callCount !== 0) {
      throw new Error(
        `可視範囲内では自動スクロールしない想定ですが、実際には ${String(callCount)} 回呼ばれました`,
      );
    }
  },
};
...
export const AutoScrollWithNearest: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (callCount !== 1) {
      throw new Error(
        `可視範囲外では自動スクロール 1 回を期待していましたが、実際には ${String(callCount)} 回でした`,
      );
    }
