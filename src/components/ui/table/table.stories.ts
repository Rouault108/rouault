...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (headers.length !== 3) {
      throw new Error(
        `3つの th 要素を期待していましたが、実際には ${String(headers.length)}個でした`,
      );
    }
...
export const WithCaption: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (!captionText.includes('2024年')) {
      throw new Error(
        `キャプションに "2024年" が含まれることを期待していましたが、実際には "${captionText}" でした`,
      );
    }
...
export const WithTfoot: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (tfootRows.length !== 1) {
      throw new Error(
        `1つの tfoot 行を期待していましたが、実際には ${String(tfootRows.length)}行でした`,
      );
    }
    ...
    if (!totalCell?.textContent.includes('合計')) {
      throw new Error(
        `tfoot に "合計" が含まれることを期待していましたが、実際には "${totalCell?.textContent ?? ''}" でした`,
      );
    }
    ...
    if (tbodyRows.length !== 3) {
      throw new Error(
        `3つの tbody 行を期待していましたが、実際には ${String(tbodyRows.length)}行でした`,
      );
    }
  },
};
...
export const MultipleTbody: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (tbodies.length !== 3) {
      throw new Error(
        `3つの tbody 要素を期待していましたが、実際には ${String(tbodies.length)}個でした`,
      );
    }
    ...
    if (allRows.length !== 6) {
      throw new Error(
        `合計 6つのデータ行を期待していましたが、実際には ${String(allRows.length)}行でした`,
      );
    }
  },
};
...
export const ColspanRowspan: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (rowHeaders.length !== 3) {
      throw new Error(
        `3つの行ヘッダー (scope="row") を期待していましたが、実際には ${String(rowHeaders.length)}個でした`,
      );
    }
  },
};
...
export const NumericData: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (!firstNumericCell?.textContent.includes('¥')) {
      throw new Error(
        '数値セルに "¥" 記号が含まれることを期待していましたが、実際には含まれていませんでした',
      );
    }
    ...
    if (rightCells.length !== 9) {
      throw new Error(
        `9つの右揃えセルを期待していましたが、実際には ${String(rightCells.length)}個でした`,
      );
    }
  },
};
...
export const BoundaryNoAriaLabel: Story = {
  parameters: {
    docs: {
      description: {
        story:
          '⚠️ **境界条件**: `aria-label` が未設定。`role="region"` にアクセシブルネームがない状態。横スクロールが発生する場合は必ず設定してください。',
      },
    },
  },
  render: () => html`
  ...
  play: async ({ canvasElement }) => {
    ...
    if (resolvedLabel !== 'Data table') {
      throw new Error(
        `aria-label="Data table"を期待していましたが、実際には "${resolvedLabel ?? 'null'}"でした`,
      );
    }
    ...
    if (container.getAttribute('tabindex') !== '0') {
      throw new Error(
        'aria-labelが設定されていないにもかかわらず、tabindex="0"が維持されるべきです',
      );
    }
  },
};
...
export const BoundarySingleRow: Story = {
  parameters: {
    docs: {
      description: {
        story:
          '⚠️ **境界条件**: データ行が 1 行のみ。最小限のテーブルが正常にレンダリングされることを確認します。',
      },
    },
  },
  render: () => html`
...
export const BoundaryHeaderOnly: Story = {
  parameters: {
    docs: {
      description: {
        story:
          '⚠️ **境界条件**: `<tbody>` に行がない空テーブル。検索結果0件などの状態に対応します。',
      },
    },
  },
  render: () => html`
  ...
  play: async ({ canvasElement }) => {
    ...
    if (rows.length !== 0) {
      throw new Error(
        `データ行が0件であることを期待していましたが、実際には ${String(rows.length)}件でした`,
      );
    }
    ...
    if (headers.length !== 3) {
      throw new Error(
        `3つのヘッダーセルを期待していましたが、実際には ${String(headers.length)}個でした`,
      );
    }
...
export const DensityToggle: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (table.density !== 'normal') {
      throw new Error(
        `初期状態は density="normal" でなければなりませんが、実際には "${table.density}" でした`,
      );
    }
    ...
    if (densityBefore === densityAfter) {
      throw new Error(
        `密度が変更されなければなりませんが、実際には "${densityAfter ?? 'null'}" でした`,
      );
    }
...
export const AccessibilityStructure: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (ariaLabel !== 'アクセシビリティ検証テーブル') {
      throw new Error(
        `aria-label="アクセシビリティ検証テーブル"を期待していましたが、実際には "${ariaLabel ?? 'null'}" でした`,
      );
    }
    ...
    if (rowHeaders.length !== 2) {
      throw new Error(
        `scope="row" のヘッダーが2つあることを期待していましたが、${String(rowHeaders.length)}個でした`,
      );
    }
...
export const AllStates: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (tables.length !== 4) {
      throw new Error(
        `4つの ui-table コンポーネントを期待していましたが、実際には ${String(tables.length)}個でした`,
      );
    }
    ...
    if (compactTable.density !== 'compact') {
      throw new Error(
        `2番目のテーブルは density="compact" でなければなりませんが、実際には "${compactTable.density}" でした`,
      );
    }
    ...
    if (tbodies.length !== 2) {
      throw new Error(
        `4番目のテーブルには2つの tbody を持つ必要がありますが、実際には ${String(tbodies.length)}個でした`,
      );
    }
  },
};
...
- touch デバイスでは hover が無効化されることを確認
  ...
  play: async ({ canvasElement }) => {
    ...
    if (!injectedStyle.textContent.includes('@media (prefers-reduced-motion: reduce)')) {
      throw new Error(
        '注入されたスタイルに prefers-reduced-motion メディアクエリが含まれていません',
      );
    }
