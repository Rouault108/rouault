...
export const CollapsedWithDropdown: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		if (items?.length !== 4) {
			throw new Error(`4つのレンダリングされた項目を期待していましたが、実際には ${String(items?.length ?? 0)}個でした`);
		}
		...
		if (hiddenMenuItems.length !== 4) {
			throw new Error(`4つのメニュー項目を期待していましたが、実際には ${String(hiddenMenuItems.length)}個でした`);
		}
	},
};
	...
	play: async ({ canvasElement }) => {
		...
		try {
			...
			if (renderedItems?.length !== 3) {
				throw new Error(`モバイル表示では3つの項目が表示されるべきですが、実際には ${String(renderedItems?.length ?? 0)}個でした`);
			}
...
export const MaxItemsEdgeCases: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		if (items1?.length !== 1) throw new Error('max-items=1 のとき、1つの項目がレンダリングされる必要があります');

		const items2 = max2.shadowRoot?.querySelectorAll('.breadcrumb-item');
		if (items2?.length !== 2) throw new Error('max-items=2 のとき、2つの項目がレンダリングされる必要があります');

		const current0 = max0.shadowRoot?.querySelector('[aria-current="page"]');
		if (!current0) throw new Error('max-items=0 は正規化され、カレントページが維持される必要があります');
	},
};
...
export const ForcedColorsMode: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		if (!trigger) throw new Error('forced-colors のシナリオでは省略トリガーが含まれている必要があります');

		const separator = host.shadowRoot?.querySelector('.breadcrumb-separator');
		if (separator?.getAttribute('aria-hidden') !== 'true') {
			throw new Error('forced-colors のシナリオでもセパレーターは aria-hidden である必要があります');
		}
	},
};
...
export const DarkMode: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		const current = host.shadowRoot?.querySelector('[aria-current="page"]');
		if (!current) throw new Error('ダークモードのストーリーでもカレントページがレンダリングされる必要があります');
	},
};
...
export const AllStates: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		if (hosts.length !== 4) {
			throw new Error(`4つの状態サンプルを期待していましたが、実際には ${String(hosts.length)}個でした`);
		}
	},
};
