...
export const DefaultExpanded: Story = {
	render: () => html`
		<div style="height: 200vh;">
			<ui-header id="header-default" sidebar-expanded>
				${fullSlotContent}
			</ui-header>
	...
	play: async ({ canvasElement }) => {
		...
		assert(
			...
			'sidebarExpanded のデフォルト値が true ではありません',
		);
		assert(
...
export const ZenModeCollapsed: Story = {
	render: () => html`
		<ui-header id="header-zen" sidebar-expanded>
			${fullSlotContent}
		</ui-header>
	`,
	play: async ({ canvasElement }) => {
		...
		assert(
			!host.sidebarExpanded,
			'sidebarExpanded プロパティが false に更新されていません',
		);
...
export const SidebarToggleEvent: Story = {
	render: () => html`
		<ui-header id="header-event" sidebar-expanded>
			${fullSlotContent}
		</ui-header>
	`,
	play: async ({ canvasElement }) => {
		...
		const collapsePromise = waitForEvent<
			CustomEvent<UiHeaderSidebarToggleDetail>
		>(host, 'ui-header-sidebar-toggle');
		host.sidebarExpanded = false;
		...
		assert(
			!collapseEvent.bubbles,
			'ui-header-sidebar-toggle がバブリングしています',
		);

		// collapsed -> expanded: イベントがdetail.expanded=trueで発火
		const expandPromise = waitForEvent<
			CustomEvent<UiHeaderSidebarToggleDetail>
		>(host, 'ui-header-sidebar-toggle');
...
export const ResponsiveVisualComparison: Story = {
	render: () => html`
		<div style="display: grid; gap: 2rem; padding: 1rem;">
			...
			>
				<p
					style="font-size: 11px; padding: 4px 8px; background: var(--bg-fill-muted); margin: 0;"
				>
					375px (Mobile)
				</p>
...
export const EmptySlots: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		const centerAssigned =
			centerSlot?.assignedElements({ flatten: true }) ?? [];
		assert(
...
export const AttributeDrivenToggle: Story = {
	render: () => html`
		<ui-header id="header-attr" sidebar-expanded>
			${fullSlotContent}
		</ui-header>
	`,
	play: async ({ canvasElement }) => {
		...
		assert(
			host.hasAttribute('sidebar-expanded'),
			'初期状態で sidebar-expanded 属性がありません',
		);
		assert(
			host.sidebarExpanded,
			'初期状態で sidebarExpanded が true ではありません',
		);
		...
		assert(
			!host.sidebarExpanded,
			'属性除去後に sidebarExpanded が false になっていません',
		);
		assert(
			!collapseEvent.detail.expanded,
			'属性除去時のイベント detail が不正です',
		);
...
export const RapidToggleReentrancy: Story = {
	render: () => html`
		<ui-header id="header-rapid" sidebar-expanded>
			${fullSlotContent}
		</ui-header>
	`,
	play: async ({ canvasElement }) => {
...
export const DynamicSlotContent: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		assert(
			assigned.length === 1,
			'初期状態で center スロットに 1 要素が配置されていません',
		);
...
export const ForcedColorsMode: Story = {
	render: () => html`
		...
		<ui-header id="header-forced" sidebar-expanded>
			${fullSlotContent}
		</ui-header>
	`,
	play: async ({ canvasElement }) => {
...
export const ReducedMotion: Story = {
	render: () => html`
		...
		<ui-header id="header-reduced-motion" sidebar-expanded>
			${fullSlotContent}
		</ui-header>
	`,
	play: async ({ canvasElement }) => {
		const host = canvasElement.querySelector<UiHeader>(
...
export const PrintStyles: Story = {
	render: () => html`
		<div>
			<ui-header id="header-print" sidebar-expanded>
				${fullSlotContent}
			</ui-header>
			<main style="padding: 2rem;">
...
export const DarkModeGlassmorphism: Story = {
	render: () => html`
		...
		>
			<ui-header id="header-dark" sidebar-expanded>
				${fullSlotContent}
			</ui-header>
			<main style="padding: 2rem;">
				${Array.from(
					...
					(_, i) => html`
						...
						>
							ダークモード背景コンテンツ #${i + 1}:
							スクロールするとヘッダーの Glassmorphism
							効果が確認できます。
							<span style="color: oklch(65% 0.15 250);"
								>カラーテキスト</span
							>も透過で確認。
						</p>
					`,
...
export const CustomBackdropSaturate: Story = {
	render: () => html`
		...
		<div style="display: grid; gap: 2rem; padding: 1rem;">
			<div>
				...
				<ui-header id="header-saturate-default" sidebar-expanded>
					${fullSlotContent}
				</ui-header>
			</div>
			...
			<div>
				...
				<ui-header
					id="header-saturate-high"
					class="high-saturation"
					sidebar-expanded
				>
					${fullSlotContent}
				</ui-header>
			...
			<div>
				...
				<ui-header
					id="header-saturate-low"
					class="low-saturation"
					sidebar-expanded
				>
					${fullSlotContent}
				</ui-header>
	...
	`,
	play: async ({ canvasElement }) => {
		const hostDefault = canvasElement.querySelector<UiHeader>(
			'#header-saturate-default',
		);
		const hostHigh = canvasElement.querySelector<UiHeader>(
			'#header-saturate-high',
		);
		const hostLow = canvasElement.querySelector<UiHeader>(
			'#header-saturate-low',
		);
