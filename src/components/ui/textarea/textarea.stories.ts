...
export const Default: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (!ta) throw new Error('textarea not found in shadow root');

        // テスト: aria-label が label プロパティと一致する
        if (ta.getAttribute('aria-label') !== 'メモ') {
            throw new Error(`Expected aria-label="メモ", got "${ta.getAttribute('aria-label') ?? 'null'}"`);
        }

        // テスト: rows 属性が設定されている
        if (ta.rows !== 3) {
            throw new Error(`Expected rows=3, got ${String(ta.rows)}`);
        }

        // テスト: disabled でない
        if (ta.disabled) throw new Error('Expected textarea to not be disabled');

        // テスト: aria-invalid="false"
        if (ta.getAttribute('aria-invalid') !== 'false') {
            throw new Error(`Expected aria-invalid="false", got "${ta.getAttribute('aria-invalid') ?? 'null'}"`);
        }
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (el.variant !== 'default') {
            throw new Error(`Expected variant="default", got "${el.variant}"`);
        }
        ...
        if (ta.classList.contains('prose')) {
            throw new Error('Default variant should not have "prose" class');
        }

        console.log('✅ All tests passed for VariantDefault story');
    },
};
...
export const VariantProse: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (el.variant !== 'prose') {
            throw new Error(`Expected variant="prose", got "${el.variant}"`);
        }
        ...
        // テスト: prose クラスが付いている
        if (!ta.classList.contains('prose')) {
            throw new Error('Prose variant should have "prose" class');
        }

        console.log('✅ All tests passed for VariantProse story');
    },
};
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (!ta.classList.contains('error')) {
            throw new Error('Error state should have "error" class');
        }

        // テスト: エラーメッセージが表示されている
        const errMsg = el.shadowRoot?.querySelector('.error-message--visible');
        if (!errMsg) throw new Error('Error message should be visible');
        if (!errMsg.textContent.includes('本文を入力してください')) {
            throw new Error(`Expected error message to include "本文を入力してください", got "${errMsg.textContent}"`);
        }

        // テスト: aria-describedby が設定されている
        const describedBy = ta.getAttribute('aria-describedby');
        if (!describedBy) throw new Error('aria-describedby should be set when error is present');
...
export const ErrorStateProse: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (!ta.classList.contains('error')) throw new Error('Should have error class');
        if (!ta.classList.contains('prose')) throw new Error('Should have prose class');

        console.log('✅ All tests passed for ErrorStateProse story');
    },
};
...
export const StateDisabled: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (!elDefault || !elProse) throw new Error('ui-textarea not found');
        ...
        for (const [id, el] of [['#disabled-default', elDefault], ['#disabled-prose', elProse]] as const) {
            const ta = el.shadowRoot?.querySelector<HTMLTextAreaElement>('textarea');
            if (!ta) throw new Error(`textarea not found in ${id}`);
            if (!ta.disabled) throw new Error(`${id}: Expected textarea to be disabled`);
        }
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        for (const [id, el] of [['#readonly-default', elDefault], ['#readonly-prose', elProse]] as const) {
            const ta = el.shadowRoot?.querySelector<HTMLTextAreaElement>('textarea');
            if (!ta) throw new Error(`textarea not found in ${id}`);
            if (!ta.readOnly) throw new Error(`${id}: Expected textarea to be readonly`);
        }

        console.log('✅ All tests passed for StateReadonly story');
    },
};
...
export const WithHelpText: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (!elDefault) throw new Error('ui-textarea not found');
        ...
        if (!errMsg) throw new Error('Error message should be visible when error is set');
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (collapsedHeight > expandedHeight) {
            throw new Error('Height should collapse when value is cleared');
        }

        console.log('✅ All tests passed for AutoGrow story');
    },
};
...
export const MaxRows: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (el.maxRows !== 5) {
            throw new Error(`Expected maxRows=5, got ${String(el.maxRows)}`);
        }
        ...
        if (!ta.classList.contains('overflow-scroll')) {
            throw new Error('Expected overflow-scroll class when content exceeds max-rows');
        }
        ...
        if (ta.classList.contains('overflow-scroll')) {
            throw new Error('overflow-scroll class should not be present when content is within max-rows');
        }

        console.log('✅ All tests passed for MaxRows story');
    },
};
...
export const AutoGrowDisabled: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (el.autoGrow) throw new Error('Expected autoGrow to be false');
        ...
        if (!ta.classList.contains('resize-vertical')) {
            throw new Error('Expected resize-vertical class when auto-grow=false and resize=vertical');
        }

        console.log('✅ All tests passed for AutoGrowDisabled story');
    },
};
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (elements.length !== 8) {
            throw new Error(`Expected 8 textareas, got ${String(elements.length)}`);
        }
        ...
        for (const id of errorEls) {
            ...
            const ta = el.shadowRoot?.querySelector<HTMLTextAreaElement>('textarea');
            if (ta?.getAttribute('aria-invalid') !== 'true') {
                throw new Error(`${id}: Expected aria-invalid="true"`);
            }
        }
        ...
        console.log('✅ All tests passed for AllVariantsAndStates story');
    },
};
...
export const EventFiring: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        if (!inputEvent) throw new Error('input event was not fired');
        ...
        if (!changeEvent) throw new Error('change event was not fired');
...
export const FocusBlurEvents: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        console.log('✅ All tests passed for FocusBlurEvents story');
    },
};
...
export const BoundaryRows1: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        console.log('✅ All tests passed for BoundaryRows1 story');
    },
};
...
export const BoundaryEmptyLabel: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        console.log('✅ All tests passed for BoundaryEmptyLabel story');
    },
};
...
export const BoundaryDisabledAndError: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        console.log('✅ All tests passed for BoundaryDisabledAndError story');
    },
};
...
export const BoundaryRequired: Story = {
  ...
  `,
    play: async ({ canvasElement }) => {
        ...
        console.log('✅ All tests passed for BoundaryRequired story');
    },
};
