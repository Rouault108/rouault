...
const meta: Meta = {
    ...
    parameters: {
        docs: {
            description: {
                component: '本文タイポグラフィと `.prose` コンテナ内の要素リズムを確認するためのストーリーです。'
            }
        }
    }
};
...
export const Default: Story = {
    render: () => renderFoundationFrame({
        title: 'Body Text',
        description: '本文では `--text-lg` と `--line-height-relaxed` を使い、読みのリズムを優先します。'
    }, html `
        ${renderFoundationSection('Body / Prose Preview', renderTokenSampleGrid([
        {
            label: '通常本文',
        ...
        ${renderFoundationSection('Typography Tokens', renderTokenValueList([
