...
export const Default: Story = {
  ...
  play: ({ canvasElement }) => {
    const h1 = canvasElement.querySelector<HTMLHeadingElement>('#heading-h1');
    const h2 = canvasElement.querySelector<HTMLHeadingElement>('#heading-h2');
    const h3 = canvasElement.querySelector<HTMLHeadingElement>('#heading-h3');
    const h4 = canvasElement.querySelector<HTMLHeadingElement>('#heading-h4');

    if (!h1 || !h2 || !h3 || !h4) {
      throw new Error('見出しサンプルが不足しています');
    }
    ...
    const h2Size = Number.parseFloat(h2Style.fontSize);
    const h3Size = Number.parseFloat(h3Style.fontSize);
    const h4Size = Number.parseFloat(h4Style.fontSize);

    if (!(h1Size > h2Size && h2Size > h3Size && h3Size >= h4Size)) {
    ...
    if (Number.parseFloat(h1Style.lineHeight) <= h1Size) {
      throw new Error('見出し line-height が不正です');
    }
