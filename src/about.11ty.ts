...
const ABOUT_SECTIONS: readonly AboutSection[] = [
  {
    ...
    heading: 'Rouaultについて',
    bodyHtml: [
      'RouaultはRuo Miyataが個人的に蓄積しているノートを、落ち着いて読める形で整理・公開するためのWebアプリケーションです。一般的なブログのように記事を時系列で積み上げるというよりも、後から読み返し、参照し、必要に応じて再構成できる知識の置き場として設計しています。',
      '扱う内容はプログラミング、数学、文学、経済学など様々な学問領域にまたがりますが、主題は分野の広さそのものではありません。断片的な情報を消費するためではなく、一定のまとまりを持った文章やノートを、本文中心に読むことを重視しています。',
