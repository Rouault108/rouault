...
2. 現在使えるキーは `label`、`order`、`sidebar` である
3. JSON なのでコメントは書けない
4. 文字列は必ずダブルクォート `"` で囲む
...
`label` は、そのディレクトリを sidebar branch、breadcrumb の directory crumb、corpus label として表示するときの構造ラベルである。`index.md` の `title` とは別物として扱う。
...
この例では、`content/program/csharp/index.md` の `title` が `C#とは何か` であっても、構造上の表示は `C#` を使う。

補足:

- `label` はディレクトリ構造名の source of truth である
- `index.md` の `title` はページ題名として別に扱う
- `label` を省略したディレクトリでは、ディレクトリ名から導出した正規化ラベルをフォールバックとして使う
