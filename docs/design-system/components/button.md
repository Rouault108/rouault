...
`pressed` は現行実装でも公開 property として存在し、内部 button に `aria-pressed` を出力します。しかし、通常表示系では `pressed` に対応する明示的な視覚差分は未実装です。したがって、`pressed` は現時点では**意味状態のみを持ち、視覚状態は未完成**です。

加えて、Forced Colors では `button[aria-selected='true']` と `button.active` に対するスタイルが存在しますが、現行実装が出力するのは `aria-pressed` です。このため、Forced Colors においても `pressed` と視覚差分の対応は未整合です。

### 2. ARIA 関連属性の pass-through

現行実装が内部 button に反映している ARIA 属性は、`aria-label`、`aria-busy`、`aria-pressed` に限定されます。`aria-expanded`、`aria-controls`、`aria-haspopup`、`aria-describedby` など、trigger 用途で有効な関係属性の pass-through は未対応です。
