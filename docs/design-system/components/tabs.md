...
`urlSync` は tabs 本体の付加機能であり、ルータ全体の状態管理やフレームワーク固有の `history.state` 形式までは所有しません。扱うのは `?tab=` とホスト配下ハッシュ解決までです。URL 更新時に履歴 state を再生成せず、既存の `history.state` は opaque に再利用します。
