...
現行契約では、狭幅環境で通常幅向けの `center` をそのまま維持しません。狭幅では `center` を非表示にし、必要な場合に限って `compact-center` による簡略文脈表示を用います。これにより、狭幅環境では start / end の操作性を優先しつつ、必要最小限の文脈表示だけを残せます。

ただし、公開契約として安定化するのは `640px` という数値そのものではなく、**狭幅では `center` が非表示になり、`compact-center` がその代替文脈表示面になり得る**という挙動です。閾値の最終所有者は header 単独ではなく、app-shell 側の responsive policy にあります。
...
`compact-center` は、狭幅時に `center` の責務を縮約して引き受ける代替文脈表示面です。これは `center` の拡張モードではなく、**狭幅で文脈表示を完全喪失しないための限定的な補助手段**です。
...
狭幅では通常幅向けの `center` は非表示になり得ます。`center` に重要操作を置く構成には依存してはなりません（MUST NOT）。`compact-center` が存在する場合は、狭幅時の代替文脈表示面としてのみ用います。`center` と `compact-center` は同時表示してはなりません（MUST NOT）。
