...
- 本文監視 root や scroll target の外部指定
- 歯抜けレベルの圧縮表示モード
- 現在地判定戦略の切り替え
- 弱い進捗表示
- 印刷モード専用 API または toc 自体の印刷専用ロジック

これらは上位レイヤまたは別コンポーネントの責務、または現時点では公開契約に含めない拡張です。

---

## 3. 公開契約

`ui-toc` は、`headers`、`activeId`、`ariaLabel`、`offsetTop` を公開入力として扱います。出力として `ui-toc-active-change` カスタムイベントを公開し、再同期のために `refresh()` 公開メソッドを持ちます。内部実装は `nav > ul > li > a` 構造、`IntersectionObserver`、tooltip、および平滑スクロール処理を用いますが、利用者は `ui-toc` を契約単位として扱います。

`headers` は描画の唯一の入力であり、`ui-toc` 自身はクライアント DOM を走査して見出し一覧を生成しません。見出しデータは外部で生成し、`Heading[]` として渡さなければなりません（MUST）。

`activeId` は現在地を示す見出し ID であり、**外部が所有する唯一の真実**として扱います。`ui-toc` 自身は `activeId` を自律的に確定しません。スクロール観測やリンククリック、または `refresh()` による再評価で次の現在地候補を検出した場合は、`ui-toc-active-change` により**外部へ通知**し、最終的な状態反映は利用者が `activeId` を更新することで行います。
...
| `ariaLabel` | property / attribute | いいえ | `nav` のアクセシブル名                     | `string`。属性名は `aria-label`、既定値は `Table of Contents` です。空文字列は未指定として扱います。可視ラベルは追加しません。       |
| `offsetTop` | property             | いいえ | 現在地判定とクリック着地に用いる上端オフセット | `number`。単位は CSS px 相当の数値です。有限かつ 0 以上でなければなりません。未指定時は現行互換の既定値を用います。                 |
...
| `level` | `number` | はい | 見出しレベル。Rouault の記事本文入力では通常 2〜6 |
...
- `level` は整数値であり、Rouault の記事本文入力では通常 2〜6 を想定します。
- `headers` の順序は本文内の見出し順と一致しなければなりません。

`level` は表示階層の算出に使用します。HTML 見出しとしての取り得る値は 1〜6 ですが、Rouault の記事本文入力としては H2〜H6 を通常入力とします。表示上は `headers` 内の最小レベルを基準に相対正規化します。たとえば H2 と H3 を含む配列では、H2 を 0、H3 を 1 として扱います。
...
`activeId` は外部が所有する controlled property です。`ui-toc` はその値に従って表示を更新しますが、自律的に現在地を確定しません。

外部から `activeId` を設定した場合、それは **確定済み現在地の反映**として扱います。したがって、この更新自体を理由に本文スクロール、URL ハッシュ更新、`ui-toc-active-change` の発火は行いません。

一方で、表示上のアクティブ項目反映と toc 内での可視性維持は行います。したがって、`activeId` の外部更新は **表示同期**であり、**移動要求や副作用実行**ではありません。
...
| `ariaLabel` | `aria-label` | あり    | `nav` のアクセシブル名に使用します。未指定時は既定値へフォールバックします。 |
| `offsetTop` | なし         | なし    | 数値入力のため property 経由でのみ受け取ります。               |
| `headers`   | なし         | なし    | property 経由でのみ受け取ります。                              |

`headers` は配列入力であり、HTML 属性文字列からの構築は契約に含みません。`offsetTop` も property で渡します。
...
`ui-toc` は `refresh(): void` を公開します。`refresh()` は、現在地候補の再評価、省略判定、tooltip 有効化、自動追従に必要な再観測・再計測を再実行します。

`refresh()` は次を満たします。

- 見出し抽出責務は持ち込まず、`headers` の再生成は行いません。
- `activeId` を自律的に確定しません。
- URL ハッシュを更新しません。
- 再評価の結果、新しい現在地候補が得られた場合に限り、`ui-toc-active-change` を再発火し得ます。
- hidden container からの表示切り替え、tab / dialog の開閉、レイアウト再確定後の再同期に使用できます。

利用者は Shadow DOM 内部を探索して直接操作してはなりません（MUST NOT）。

### 3.8 責務範囲

責務範囲には、見出し一覧の描画、相対階層の正規化、現在地の視覚表示、クリックによるアンカー移動、現在地変更イベントの通知、長文省略時の tooltip 有効化、Reduced Motion / Forced Colors への適応、`nav` アクセシブル名の外部入力、`offsetTop` に基づく現在地判定とクリック着地、`refresh()` による再同期を含みます。

一方で、見出しデータの抽出、記事本文側の `scroll-margin-top` 設計、ルーターとの同期、ページ全体のレイアウト確定、tooltip 文言の多言語化、本文監視 root / scroll target の外部指定、歯抜けレベルの圧縮表示モード、現在地判定戦略の切り替え、弱い進捗表示、印刷モード専用 API は責務に含めません。

---

## 4. 状態モデル

`ui-toc` の主要状態は、**有効な見出し集合を持つか**、**外部から確定済みの `activeId` が与えられているか**、**各項目が省略表示されているか**、**現在地候補を外部へ提案すべき状況か**によって読み分けます。
...
ただし、省略判定と tooltip 有効化はレイアウト計測後に確定するため、初回描画時点では未確定であり得ます。表示直後や非表示状態からの表示切り替え直後には、tooltip の有効 / 無効が遅れて反映される場合があります。利用者は tooltip の有効 / 無効を初回同期描画のみで断定してはなりません（MUST NOT）。
...
* ランドマークは `nav` です。
* `nav` の `aria-label` は公開入力 `ariaLabel` に従います。未指定時の既定値は `Table of Contents` です。
* 可視ラベルは追加しません。
...
表示上の階層は `headers` 内の最小 `level` を基準に相対値へ正規化します。したがって、H2 のみの配列でも全項目は `level: 0` として表示されます。現行契約では、H2・H4・H6 のような歯抜け入力についても、その差分を圧縮せず表示へ反映します。

つまり、現行の正規化は **最小値起点の平行移動**であり、**連続レベルへの再マッピング**ではありません。ただし、この挙動は入力意味の保存を優先した選択であり、視覚可読性を最優先する圧縮表示を将来否定するものではありません。
...
tooltip は、**省略が起きている項目**にのみ有効化します。アクティブ項目も省略されている場合は tooltip を利用可能とします。これは、レイアウト安定性を維持しつつ全文参照の補助手段を残すためです。

ただし、tooltip は補助手段であり、hover を前提としない環境で主要な全文参照経路であることまでは契約しません。
...
また、現在地判定に用いる観測オフセットは、クリック移動時の着地オフセットと意味上一貫していなければなりません。公開入力 `offsetTop` が指定されている場合、`ui-toc` は現在地判定とクリック着地の両方に同じ値を用います。`offsetTop` が未指定の場合は、現行互換の既定値を用います。可視見出しが一時的に 0 件であっても、現在地はこの規則に従って直前見出しへ解決します。
...
- 公開入力 `offsetTop` を反映した着地位置へ平滑スクロールします。
...
アクティブ項目が toc 自身の可視範囲から外れた場合のみ、toc はその項目を再び可視範囲へ収めるための最小限の追従スクロールを行います。可視範囲内にある場合は、自動追従しません。常時センタリングや強制追従は契約に含めません。

自動追従の対象となるスクロール領域の決定方法、および可視判定の内部アルゴリズムは公開契約に含めません。ただし、利用者は **「可視範囲外のアクティブ項目のみを、最小移動で再表示する」** という振る舞いを前提に扱えます。
...
また、省略判定はレイアウト計測後に確定するため、tooltip の有効 / 無効も描画直後には未確定であり得ます。なお、tooltip は全文参照の補助手段であり、表示不能な環境でもリンクナビゲーション自体が成立することを優先します。
...
最小レベルが 2 であっても、それを基準に相対階層 0 として表示します。H1 の存在は前提にしません。

### 9.5 歯抜けレベル

H2・H4・H6 のような歯抜け入力では、現行契約上の相対階層は 0・2・4 となります。レベル差は圧縮しません。そのため、入力構造は保存されますが、文書によってはインデント差が大きく見える場合があります。
...
| `NormalizedLevels`                        | 最小レベル基準の相対正規化が成立し、H2 と H3 を含む入力で H2 を 0、H3 を 1 として扱えること。                                         |
...
1. **`activeId` は controlled property とする。** `ui-toc` は現在地候補を通知するが、自律的に確定しない。
2. **`ui-toc-active-change` は現在地候補通知とする。** これは内部状態確定通知ではなく、親への提案イベントである。
3. **現在地は観測オフセット直前の見出しを優先する。** 単なる `headers` 先頭可視一致は採らない。
4. **有効なクリックのみ副作用を伴う。** ターゲット不在クリックでは、表示・URL・イベントのいずれも変えない。
5. **`headers` は immutable input とする。** 破壊的更新に依存しない。
...
11. **`ui-toc` は document / window 前提の読書用コンポーネントとして扱う。** 汎用スクロール root 対応は現時点の公開契約に含めない。
...
### 13.9 アクティブかつ省略中の項目に対する tooltip 維持
...
### 13.10 coarse pointer における 44px 実効タッチターゲット

現行実装が CSS で常時保証している最小値は 24px です。本契約書では 44px を正式契約としたため、実装と Storybook の双方で再検証が必要です。

### 13.11 同一 hash の重複 push 防止

現行実装では有効クリック時の URL 更新はありますが、同一 hash への重複 push 防止が契約どおりかは未固定です。

### 13.12 `ariaLabel` 公開入力と `nav` アクセシブル名の外部化

本契約書では、`nav` のアクセシブル名を公開入力 `ariaLabel` で受け取れるものとして固定しました。未指定時の既定値は `Table of Contents` であり、可視ラベルは追加しません。

したがって、現行実装には少なくとも次の未対応があります。

- `ariaLabel` property / attribute の公開
- `aria-label` attribute への reflect
- `nav` 要素への反映
- 未指定時の既定値フォールバック
- Storybook における既定値 / 明示指定 / 空文字列時の確認観点

現行実装と補助文書が `nav[aria-label="Table of Contents"]` 固定前提のままであれば、この契約には未追随です。

### 13.13 `offsetTop` 公開入力とオフセット意味の単一化

本契約書では、現在地判定とクリック着地に用いる上端オフセットを公開入力 `offsetTop` として固定しました。`offsetTop` が指定されている場合、`ui-toc` は現在地判定とクリック着地の両方に同じ値を用い、未指定時のみ現行互換の既定値へフォールバックします。

したがって、現行実装には少なくとも次の未対応があります。

- `offsetTop` property の公開
- 有限かつ 0 以上の数値としての入力検証
- 現在地判定オフセットとクリック着地オフセットの単一入力化
- `--header-height` を既定値算出の参照へ下げる整理
- Storybook における既定値 / 明示値 / 0 / 不正値の確認観点

現行実装が `--header-height` と内部固定余白に依存し、公開 API としてはオフセットを受け取れないままであれば、この契約には未追随です。

### 13.14 `refresh()` 公開メソッドによる明示的再同期

本契約書では、再同期のための公開メソッド `refresh(): void` を正式契約に含めました。`refresh()` は、現在地候補の再評価、省略判定、tooltip 有効化、自動追従に必要な再観測・再計測を再実行しますが、`headers` の再生成、`activeId` の自律確定、URL ハッシュ更新は行いません。

したがって、現行実装には少なくとも次の未対応があります。

- `refresh()` 公開メソッドの定義
- hidden container からの表示切り替え後の再同期経路
- tab / dialog 開閉後の再観測・再計測の再実行
- 再評価の結果としてのみ `ui-toc-active-change` を再発火し得るという意味整理
- Storybook における再同期メソッド呼び出し観点の追加

現行実装が初回接続時や通常スクロール時の内部同期だけを前提にし、利用者からの明示的再同期手段を持たないままであれば、この契約には未追随です。

### 13.15 `toc.ts` の JSDoc / 実装コメントの旧契約
...
### 13.16 `toc.stories.ts` の docs / play の旧契約
