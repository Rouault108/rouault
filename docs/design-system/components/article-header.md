...
- `article-header` の末尾要素と下端罫線の距離は、**container 自身の `padding-block-end`** により確保します。
- したがって、末尾の逃がしは `tags-row` や `metadata-list--secondary` など個別要素の `margin-block-end` ではなく、**`article-header` 側の境界契約**として扱います。
...
- stack 内部の段間は、**要素固有 margin の偶発ではなく、隣接要素間 selector により制御**します。
- 段間は次を基準とします。
  - タイトル → 主要メタデータ: `20px`
