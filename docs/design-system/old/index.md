...
Rouaultは、個人的なメモ（ノート）を閲覧するためのLitで作成される専用Webアプリケーションで、「没入して読む」ことのできるデザインを目指します。

Linearなどの高密度プロダクトを参照しつつ、最高のリーディング体験は**「UIの透明化」**——すなわちコンテンツへの主役交代によってもたらされると定義します。我々は本文エリアにおける装飾的な枠線を最小化しますが、ナビゲーションや独立した情報ブロックにおいては、明確な構造を示すために繊細な枠線を機能的に使用します。
...
| 区分         | 内容                                                           |
| ------------ | -------------------------------------------------------------- |
| **対象読者** | デザイナー、フロントエンド実装者、レビュー担当者               |
| **適用対象** | `src/` 配下のRouault UI全体（Web Components / 通常DOMの両方）  |
| **非対象**   | マーケティングページ、外部サイト埋め込みUI、実験用プロトタイプ |

### 品質指標 (SLO)

デザイン品質を定性的表現だけで終わらせないため、以下を運用上の目標値とします。

| 指標                         | 目標                                    |
| ---------------------------- | --------------------------------------- |
| **アクセシビリティ自動検査** | 主要フローで `axe-core` 重大違反 0      |
| **操作応答性 (INP)**         | 主要操作のINP p75 < 200ms               |
| **視覚的安定性 (CLS)**       | 主要ページで CLS < 0.1                  |
| **読書面の可読性**           | 本文テキストのコントラスト比 4.5:1 以上 |
...
| **状態ラベル文言**      | 日本語固定 | `required` などの状態ラベルは英語併記せず「必須」のように日本語へ統一。視覚テキストとスクリーンリーダー読み上げ（`aria-label`）を一致させ、認知負荷を下げる。                |

> **設計意図**: 個人プロジェクトとして、日本語タイポグラフィ（Noto Sans JP、和文メトリクス等）に最適化することで、複雑性を排除し、保守性と品質を最大化します。

### デザイン原則

以下の4つの原則は、下記の優先順位を持ちます。原則間で衝突が生じた場合、上位の原則が優先されます。
...
| **Fill**                    |          |                                                    |                                              |                                                                                                                                                                     |
| `--bg-fill-muted`           | Semantic | `oklch(96% 0 0)`                                   | `oklch(9% 0 0)`                              | 入力フォーム、コードブロック背景                                                                                                                                    |
...
| `--control-height-lg`  | `40px` | _Deprecated_: 原則として使用しない。特別に強調が必要な場合のみ例外的に許可。          |
...
| `--space-4`  | 1rem (16px)    | **標準**: パラグラフ間、入力フォーム間。_ベースライン基本単位_     |
...
| トークン                | 定義                                                     | 用途                                                                          |
| ----------------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `--border-subtle`       | `var(--border-default)`                                  | **標準境界色のエイリアス**。コンポーネント実装時は色トークンとして参照。      |
| `--border-style-subtle` | `var(--border-width) solid var(--border-subtle)`         | **標準境界線スタイル**。カード、ダイアログ、入力フィールドに使用。            |
...
- **`--border-subtle` (Color Token Alias)**: `--border-default` の意味論的エイリアス。色のみを参照する場合に使用。
- **`--border-style-subtle` (Style Token)**: `var(--border-width) solid var(--border-subtle)` のショートハンドとして使用。
...
.glass-panel {
  ...
  box-shadow:
    ...
    /* 上部の光（ハイライト） */ inset 0 0 0 1px oklch(100% 0 0 / 0.03); /* 全周の微細な境界 */
}
...
| `--focus-ring-radius`       | `var(--radius-sm)` | リングの角丸。全コンポーネントの既定値。                          |
...
**運用ルール (Porting Governance)**

| 項目                 | ルール                                                                                             |
| -------------------- | -------------------------------------------------------------------------------------------------- |
| **上流追従**         | 四半期ごとに参照元の主要変更（a11y・キーボード操作・フォーム制御）を棚卸しし、必要差分を取り込む。 |
| **セキュリティ修正** | 参照元にセキュリティ関連修正が出た場合は優先取り込み対象とし、72時間以内に影響評価を記録する。     |
| **ライセンス管理**   | 参照した実装のライセンス情報と出典URLを `docs/` 配下に記録し、更新時に再確認する。                 |
| **差分記録**         | ポーティング後に意図的に変更したロジックは「変更理由・互換影響」をPR本文に残す。                   |
...
2.  **完全なカプセル化 (Full Encapsulation)**:
    ...
    - **例外（意味論優先）**: `dl/dt/dd` のように、ネイティブの関係性を親DOMへ直接統合すること自体がアクセシビリティ要件となる場合は、Light DOM 実装を許可します。理由は、支援技術に対して「用語と定義」の構造を崩さず伝えることを、カプセル化より優先するためです。
...
:host {
  ...
  --ui-code-block-radius-bottom: var(--radius-md);
  border-radius: var(--ui-code-block-radius-top) var(--ui-code-block-radius-top)
    var(--ui-code-block-radius-bottom) var(--ui-code-block-radius-bottom);
}
...
| **レイアウト構成** | **[左]** サイドバートリガー & ロゴ <br> **[中央]** パンくずリスト（Context） <br> **[右]** 検索トリガー & ユーザーメニュー <br> ※ モバイル時（`<= --bp-sm`）のパンくずは、ヘッダー領域のS/N比を維持するため **「ルート / ... / 現在地」** の最小構成へ自動凝縮します。               |
...
小さな操作（ボタンクリック等）のローディング状態を示します。詳細仕様は `components.md` のローディング/スケルトンセクションを参照してください。

| プロパティ                 | 値                                               | 説明                                                   |
| -------------------------- | ------------------------------------------------ | ------------------------------------------------------ |
| **サイズ**                 | `default`: `1em` / `lg`: `var(--icon-xl)` (32px) | テキストサイズに追従。大サイズはオーバーレイ用         |
| **SVG構造**                | `viewBox="0 0 50 50"`, `cx="25" cy="25" r="20"`  | スケーラブルなキャンバス。ストローク余白を確保         |
| **ストローク幅**           | `10%`（viewBox比 = 5単位）                       | コンテナサイズに比例し、全サイズで視覚的一貫性を維持   |
| **色**                     | `currentColor`                                   | 親要素のテキスト色を継承                               |
| **回転アニメーション**     | `rotate 2s linear infinite`                      | 継続的なループに適した周期（UIトランジションとは独立） |
| **ダッシュアニメーション** | `dash 1.5s ease-in-out infinite`                 | ストローク伸縮による進行感の演出                       |
