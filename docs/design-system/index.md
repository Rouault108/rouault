...
1. 変更提案PRに「背景・変更内容・影響範囲・移行有無」を記載する。
2. 互換性影響がある場合は `CHANGELOG.md` に移行手順を同時追加する。
3. `index.md` と `components.md` の参照整合を確認してからマージする。
4. 緊急修正は先に反映してよいが、48時間以内に理由と恒久対策を追記する。

### コンポーネント成熟度

`components.md` に記載する各コンポーネントは、以下の成熟度ラベルを持つものとします。

| ステータス | 意味 |
|-----------|------|
| **Draft** | API/見た目が変わる可能性が高い |
| **Beta** | 本番利用可能だが破壊的変更の可能性あり |
| **Stable** | 互換性を維持し、破壊的変更時は移行手順必須 |
| **Deprecated** | 廃止予定。代替手段と廃止バージョンを必ず併記 |
