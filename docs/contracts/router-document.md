...
- Type: Normative
- Source of truth: SSR出力、`router-document-host`、router document helpers、対応するSSR/browser tests
...
- `router-document-host`のlight DOM内に本文boundaryが存在すること。
...
- `router-document-host:content-dom-replaced` event。
- `router-document-host:navigation-committed` event。
- `getContentRoot()`の安定した本文root。

### Events

- `router-document-host:content-dom-replaced`は本文DOM差し替えが完了したことを通知する。
- `router-document-host:navigation-committed`はrouter documentとshell commitが観測可能になったことを通知する。
...
- `router-document-host`は本文light DOMを保持し、本文rootをshadow DOM内だけに閉じ込めてはならない。
...
- SSR layoutは`main#main-content`、`router-document-host`、announcement regionを出力する。
