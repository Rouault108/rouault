...
| code-group          | container | true            | recommended-top-level  | none               | none                                        | none              | none                          | `aria-label`, `sync-scope`                                                                                                                                                | `build/remark/directives/payload/normalize-surface-payload.ts`, `build/remark/directives/output/**`                    |
...
| preview-sandbox     | container | true            | specialized-child-only | `code-preview`     | `preview-html`, `preview-css`, `preview-js` | `preview`         | 1                             | `iframe-title`, `base-url`, `allow-js`, `activation-policy`, `height-mode`, `content-layout`, `allow-forms`, `allow-downloads`, `allow-pointer-lock`, `allow-popups`, `height`, `max-height` | `build/remark/directives/payload/normalize-preview-payload.ts`, `build/remark/directives/validator/**`                 |
...
`preview-sandbox`の`content-layout`はexact lowercaseの`stage` / `flow`だけを指定できる。未指定時は`stage`として表示されるが、authoring sourceへ既定属性を補う必要はない。
