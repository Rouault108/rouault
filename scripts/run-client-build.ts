...
import type { RunBuildStep } from './run-build-process.js';

const CLIENT_BUILD_STEPS = [
  {
    label: 'codegen:icons',
    pnpmArgs: ['run', 'codegen:icons'],
  },
  {
    label: 'prepare:static-font-assets',
...
const buildMetadata = (() => {
  try {
    ...
    console.error(
      '[client-build] invalid build metadata:',
...
for (const step of CLIENT_BUILD_STEPS) {
  const invocation = (() => {
    try {
      ...
      if (error instanceof RunBuildProcessConfigurationError) {
        console.error('[client-build] invalid build process configuration:', error.message);
        process.exit(1);
      }
  ...
  if (result.error) {
    console.error(
      `[client-build] build step could not be started: ${step.label} (${invocation.strategy})`,
  ...
  if (result.status !== 0) {
    console.error(
      `[client-build] build step failed: ${step.label} (${invocation.strategy})`,
